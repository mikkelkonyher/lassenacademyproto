...
- `bruno/e2e-flow/` — 30-step sequential flow. Steps 1–13: Login → forum post/comment CRUD, each mutation followed by a GET verify. Steps 14–19: news CRUD. Steps 20–22: course purchase + idempotency. Steps 23–30: signed Mux playback — resolve lesson IDs, public lesson needs no token, anonymous denied (401), owner granted (JWT claims asserted: RS256, `kid`, `aud` `v`/`s`, matching `sub`, future `exp`), then two calls straight to `stream.mux.com` proving it returns **403 without a token and 200 with ours**, plus 400/404 input handling.
  ...
  - Step 21 deliberately does **not** assert `payment_provider: mock` — step 20 is idempotent and the test user's row now says `stripe`, written by the real sandbox checkout.
