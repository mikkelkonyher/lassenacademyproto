...
/**
 * Edge Function: delete-forum-comment
 *
 * Deletes a forum comment with server-side ownership verification.
 * Also deletes associated notifications.
 * Security: JWT auth, ownership check, rate limiting.
 * Rate limit: max 15 deletions per hour per user.
 */
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (existingComment.user_id !== user.id) {
      return jsonError("You can only delete your own comments", "FORBIDDEN", 403, corsHeaders);
    }
    ...
    if (deleteError) {
      ...
      return jsonError("Failed to delete comment", "DELETE_FAILED", 500, corsHeaders);
    }
    ...
    await reportError("delete-forum-comment", err); // report to Sentry (backend project)
