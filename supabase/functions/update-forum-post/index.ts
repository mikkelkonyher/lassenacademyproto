...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (existingPost.user_id !== user.id) {
      return jsonError("You can only edit your own posts", "FORBIDDEN", 403, corsHeaders);
    }
    ...
    if (updateError) {
      ...
      return jsonError("Failed to update post", "UPDATE_FAILED", 500, corsHeaders);
    }
    ...
    await reportError("update-forum-post", err); // report to Sentry (backend project)
